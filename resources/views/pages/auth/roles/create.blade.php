...
@section('content')
<div class="card">
    <form action="{{ route('roles.store') }}" id="form-submit" method="post">
        @csrf
                    ...
                    <tbody>
                        ...
                        @foreach ($permissions as $permission)
                            <tr>
                                ...
                                <td>
                                    <div class="form-check form-check-inline">
                                        <input type="checkbox" class="form-check-input me-1" name="permission[]" id="read-{{ $permission->id }}" value="{{ $permission->id }}" @if (old('permission') && in_array($permission->id, old('permission'))) checked @endif>
                                ...
                                <td>
                                    @if($permission->create)
                                        <div class="form-check form-check-inline">
                                            <input type="checkbox" class="form-check-input me-1" name="permission[]" id="create-{{ $permission->id }}" value="{{ $permission->create->id }}" @if (old('permission') && in_array($permission->create->id, old('permission'))) checked @endif>
                                ...
                                <td>
                                    @if($permission->update)
                                        <div class="form-check form-check-inline">
                                            <input type="checkbox" class="form-check-input me-1" name="permission[]" id="update-{{ $permission->id }}" value="{{ $permission->update->id }}" @if (old('permission') && in_array($permission->update->id, old('permission'))) checked @endif>
                                ...
                                <td>
                                    @if($permission->delete)
                                        <div class="form-check form-check-inline">
                                            <input type="checkbox" class="form-check-input me-1" name="permission[]" id="delete-{{ $permission->id }}" value="{{ $permission->delete->id }}" @if (old('permission') && in_array($permission->delete->id, old('permission'))) checked @endif>
                    ...
                    if (response.status == "success") {
                        ...
                        switch (redirect) {
                            case 'new':
                                ...
                                break;
                            case 'edit':
                                window.location.href = `{{ route('roles.index') }}/${response.data.encryptId}/edit`;
                                break;
                            default:
